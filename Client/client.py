...
def Pack_File():
    ...
    Create_BSF()
    Filesize = os.path.getsize('Temp/{}.bsf'.format(Filename))
    Filepath = 'Temp/{}.bsf'.format(Filename)
    os.remove('Temp/{}.zip'.format(Filename))
    return FileObj(FileName=Filename+'.bsf', FilePath=Filepath, FileSize=Filesize)
...
def Create_Backup():
    file = Pack_File()
    args = [file]
    daemon = threading.Thread(target=File_Transfer, args=(args))
    daemon.daemon = True
    daemon.start()
